...
def generar_pdf_cedivet(estudio_id, paciente, especie, raza, fecha, medico, sexo, edad, tipo_estudio, datos_estudio, observaciones_txt):
    ...
    if os.path.exists("marca_agua.jpg"):
        c.drawImage("marca_agua.jpg", 0, 0, width=612, height=792)
    ...
    c.drawString(45, y, "NO. ESTUDIO:")
    c.drawString(45, y - 12, "PACIENTE / IDENT.:")
    c.drawString(45, y - 24, "ESPECIE:")
    c.drawString(45, y - 36, "RAZA:")

    c.setFont("Helvetica", 8)
    c.drawString(135, y, estudio_id)
    c.drawString(135, y - 12, paciente)
    c.drawString(135, y - 24, especie)
    c.drawString(135, y - 36, raza)

    c.setFont("Helvetica-Bold", 8)
    c.drawString(340, y, "FECHA:")
    c.drawString(340, y - 12, "MEDICO SOLICITANTE:")
    c.drawString(340, y - 24, "SEXO:")
    c.drawString(340, y - 36, "EDAD:")

    c.setFont("Helvetica", 8)
    c.drawString(445, y, fecha)
    c.drawString(445, y - 12, medico)
    c.drawString(445, y - 24, sexo)
    c.drawString(445, y - 36, edad)
    ...
    # 3. IMPRESIÓN DE BLOQUES DINÁMICOS EN PDF
    ...
    estudio_clean = estudio_id.strip().replace(' ', '_')
    tipo_clean = tipo_estudio.strip().replace(' ', '_').replace('(', '').replace(')', '').replace('+', 'Y')
    especie_clean = especie.strip().replace(' ', '_')
    paciente_clean = paciente.strip().replace(' ', '_')
